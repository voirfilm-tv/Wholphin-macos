  ...
  return `<div class="app-shell ${drawerExpanded ? 'drawer-expanded' : ''}">
    ...
    <aside class="side-nav" aria-label="Navigation principale">
      ...
      <nav class="nav-items">
        ...
        ${visibleViews.map((view) => navButton({ label: view.Name, icon: view.CollectionType === 'movies' ? '▣' : view.CollectionType === 'tvshows' ? '▤' : view.CollectionType === 'music' ? '♫' : view.CollectionType === 'photos' ? '▧' : '□', active: route.name === 'library' && route.parentId === view.Id, key: `library:${view.Id}`, extra: `data-library-id="${attribute(view.Id)}" data-library-title="${attribute(view.Name)}" data-library-type="${attribute(view.CollectionType ?? '')}"` })).join('')}
        ${moreViews.length ? navButton({ label: 'Plus', icon: '⌄', key: 'more', extra: 'data-action="toggle-more"' }) : ''}
        <div class="nav-more" data-nav-more hidden>${moreViews.map((view) => navButton({ label: view.Name, icon: '□', active: route.name === 'library' && route.parentId === view.Id, key: `library:${view.Id}`, extra: `data-library-id="${attribute(view.Id)}" data-library-title="${attribute(view.Name)}" data-library-type="${attribute(view.CollectionType ?? '')}"` })).join('')}</div>
        ${navButton({ label: 'Favoris', icon: '♥', route: 'favorites', active: routeActive(route, 'favorites'), key: 'favorites' })}
        ${navButton({ label: 'Découvrir', icon: '✦', route: 'discover', active: routeActive(route, 'discover') || routeActive(route, 'discoverItem'), key: 'discover' })}
    ...
    <main class="main" id="main-content"><header class="topbar"><h1>${escapeHtml(title)}</h1><div class="top-actions">${showClock ? '<time class="clock" data-clock></time>' : ''}<button class="btn icon menu-toggle" data-focusable="true" data-focus-zone="topbar" data-focus-key="topbar:menu" data-action="toggle-drawer" aria-label="Ouvrir le menu">☰</button></div></header><div class="route-content">${content}</div></main>
  </div>`;
}
